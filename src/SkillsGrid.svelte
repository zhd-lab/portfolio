<script>
    ...
    function defineBadgeColor(sectionColor) {
        ...
        if (sectionColor === "#7fb37fc7")
            // dev web
            return "success";
        if (sectionColor === "#50789ea8")
            // inté resp
            return "primary";
        if (sectionColor === "#923f3f7e")
            // algo
            return "danger";
    }
    ...
    <div class="skills-presentation">
        {#each sections as section, index}
            ...
            >
                ...
                <div class="keywords">
                    {#each section.keywords as key, i}
                        <div
                        ...
                        >
                            <div
                                ...
                                color={hoveredIndex === index
                                    ? defineBadgeColor(section.color)
                                    : badgeDefaultColor()}
                                style={`--delay:${i * 80}ms`}
                            >
                                {key}
...
------------------------------------------------------------- */
    ...
    .skills-presentation::before {
        ...
        background-image: radial-gradient(
                circle,
                color-mix(in srgb, var(--bg-color) 70%, transparent) 0%,
                transparent 55%
            ),
            radial-gradient(
                circle,
                color-mix(in srgb, var(--bg-color) 50%, transparent) 0%,
                transparent 60%
            ),
            radial-gradient(
                circle,
                color-mix(in srgb, var(--bg-color) 35%, transparent) 0%,
                transparent 65%
            );
...
------------------------------------------------------------- */
    ...
    /* Transition douce + écoute du délai */
    .dynamic-badge {
        padding: 0.1vh 0.8vh 0.1vh 0.8vh;
        font-size: 1.4vh;
    ...
    /* Quand on survole la section (donc la carte a --hover-color) */
    .section-card[style*="--hover-color"] .dynamic-badge {
        background-color: var(--hover-color) !important;
        color: white !important;
        transition-delay: var(--delay); /* 📌 cascade ! */
    }
</style>
